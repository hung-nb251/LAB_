...
from axia_filter_candidates import CausalForceFilter
...
DEADBAND_N      = 1.5          # Ngưỡng deadband lực (N)
...
class AxiaROS2Node(Node):
    ...
    def __init__(self, on_data_cb=None):
        ...
        self._pub_calibration_sample = self.create_publisher(
            String, '/axia/calibration_sample', 10)
        ...
        # P1-selected causal filter. The legacy ForceFilter class remains above
        # for replay/tests, but runtime uses packet timestamps and the adaptive
        # median-3 candidate. It runs before gravity compensation/deadband.
        self._filter_name = 'median3_adaptive'
        self._filter = CausalForceFilter(self._filter_name, gap_reset_sec=0.2)
        ...
        while True:
            try:
                ...
                if len(data) == 24:
                    ...
                    self._process_raw_data(F3, timestamp=arrival)
    ...
    def _process_raw_data(self, F3: np.ndarray, timestamp=None):
        ...
        F_filt = self._filter.update(F3, timestamp)
        if F_filt is None:
            # Invalid/out-of-order packets reset the filter and must not
            # republish stale force as fresh data.
            return
    ...
    def compensate(self, F_raw_3):
        ...
        # Diagnostic only: record the exact pre-deadband reference and transform.
        # A missing TF is explicitly invalid for frame-based calibration.
        diagnostic = dict(schema=1, stamp_ns=self.get_clock().now().nanoseconds,
                          frame=BASE_FRAME, tf_valid=R_ws is not None,
                          calibrated=self._is_calibrated,
                          filter_name=self._filter_name,
                          force_pre_deadband=F_base.tolist(),
                          force_post_deadband=F_out.tolist(),
                          filtered_sensor_force=F_raw_3.tolist(),
                          bias_sensor_n=self._bias_F.tolist(),
                          rotation_base_to_sensor=R_ws.tolist() if R_ws is not None else None,
                          mass_kg=PAYLOAD_MASS_KG, deadband_n=self._deadband)
        self._pub_calibration_sample.publish(String(data=json.dumps(diagnostic)))
        return F_out
    ...
    def set_filter_alpha(self, alpha):
        # Kept for the existing UI signal/API. P1 runtime parameters are
        # fixed by the selected candidate; changing the legacy alpha must not
        # silently turn the adaptive filter into a different algorithm.
        self.get_logger().info(
            f'Ignoring legacy EMA alpha={float(alpha):.3g}; '
            f'runtime filter remains {self._filter_name}')
...
class AxiaSensorUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, ros_node: AxiaROS2Node):
        ...
        ctrl.addSpacing(30)
        lbl = QtWidgets.QLabel('Force Filter (P1):')
        ...
        self.combo.addItems([
            'Median-3 adaptive (P1) — Dang dung',
            'Legacy median-5 / EMA (chi doi chieu)',
        ])
    ...
    def _on_filter(self, idx):
        if idx != 0:
            ...
            self.lbl_status.setText('ℹ️ Runtime dang co dinh Median-3 adaptive (P1)')
        self.worker.set_filter(1.0)
